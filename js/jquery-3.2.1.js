...
var plana = arr.flat? función (matriz) {
	...
	devuelve arr.concat.apply ([], matriz);
};
...
var i,
	...
	booleans = "verificado | seleccionado | asíncrono | enfoque automático | reproducción automática | controles | diferir | deshabilitado | oculto |" +
		"ismap | loop | múltiple | abierto | readonly | requerido | alcance",
	...
	rcombinators = nuevo RegExp ("^" + espacio en blanco + "* ([> + ~] |" + espacio en blanco + ")" + espacio en blanco +
		"*"),
	rdescend = nuevo RegExp (espacio en blanco + "|>"),
	...
	fcssescape = function (ch, asCodePoint) {
		...
		// Otros caracteres ASCII potencialmente especiales se escapan de la barra invertida
		devuelve "\\" + ch;
	},
	...
	si (! semilla) {
		...
		if (documentIsHTML) {
			...
			if (support.qsa &&
				...
				// En tales casos, solucionamos el comportamiento al prefijar cada selector en el
				...
				if (nodeType === 1 &&
					...
					if (newContext! == context ||! support.scope) {
						...
						// Capture la ID de contexto, configurándola primero si es necesario
	...
	función de retorno (elem) {
		...
		if ("formulario" en elem) {
			...
			if (elem.parentNode && elem.disabled === false) {
				...
				// Soporte: IE 6 - 11
				// Use la propiedad de acceso directo isDisabled para verificar antepasados ​​del conjunto de campos deshabilitados
		...
		// Algunas víctimas quedan atrapadas en nuestra red (etiqueta, leyenda, menú, seguimiento), pero no debería
...
setDocument = Sizzle.setDocument = function (nodo) {
	...
	// Regresar temprano si el documento no es válido o si ya está seleccionado
	// Soporte: IE 11+, Edge 17-18+
	// IE / Edge a veces arroja un error "Permiso denegado" cuando se compara estrictamente
	...
	support.scope = afirmar (función (el) {
		...
		return typeof el.querySelectorAll! == "undefined" &&
			! el.querySelectorAll (": ámbito fieldset div") .length;
	});
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		afirmar (función (el) {
			...
			// Esto es para probar el tratamiento de IE de manera no explícita
...
getText = Sizzle.getText = función (elem) {
	...
	if (! nodeType) {
		...
		// uso de innerText eliminado por coherencia de nuevas líneas (jQuery # 11153)
...
Expr = Sizzle.selectors = {
	...
	preFiltro: {
		...
		"PSEUDO": función (coincidencia) {
			...
			// Elimina el exceso de caracteres de argumentos sin comillas
	...
	filtro: {
		...
		"CLASE": función (className) {
			...
			patrón de retorno ||
				(patrón = nuevo RegExp ("(^ |" + espacio en blanco +
		...
		"ATTR": función (nombre, operador, cheque) {
			función de retorno (elem) {
				...
				operador de retorno === "="? resultado === comprobar:
					...
					operador === "^ ="? verifique && result.indexOf (verifique) === 0:
		...
		"NIÑO": función (tipo, qué, _argumento, primero, último) {
			...
			volver primero === 1 && último === 0?
				...
				función (elem, _context, xml) {
					var cache, uniqueCache, outsideCache, node, nodeIndex, start,
					...
					si (padre) {
						...
						if (reenviar && useCache) {
							...
							// ... de una manera amigable para gzip
							nodo = padre;
